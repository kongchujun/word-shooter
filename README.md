...
| **访问** | 谁在用:按 IP 看请求次数、设备(iPad/电脑/爬虫)、首次和最近访问、最近打开的页面;下面是逐条请求流水。公网 IP 会标红提醒 |
...
下载失败不会让构建挂掉 —— 只是这一版没有归属地,页面自动隐藏那一列。数据文件用 `go:embed` 打进二进制,
所以 `deploy.sh` 仍然只下载一个文件,发布包从 6.9MB 涨到约 18MB。
...
游戏和后台都支持**中文 / English**,右上角(游戏是左侧栏底部)可以切。选择记在浏览器 `localStorage` 里,
没选过的话按浏览器语言猜,猜不出用中文。

文案在 [`web/src/i18n/`](web/src/i18n/):

```
index.ts   t() 取词、语言检测、切换
zh.ts      中文,**文案的源** —— 加新词条先加这里
en.ts      English
```

`en.ts` 的类型是 `Record<keyof typeof zh, string>`,所以**漏翻一条就编译不过**,
不会出现"上线才发现某个按钮还是中文"。

加一门语言:复制 `en.ts` 改成 `ja.ts`,在 `index.ts` 的 `LOCALES` 和 `CATALOGS` 里各加一行。

两个已知的实现细节:

- **切语言会整页重载**。这套界面全是 `innerHTML` 模板字符串,没有响应式框架,局部重绘要挨个调各页的
  render 很容易漏。换语言是低频动作,重载最稳。
- **文案不能存成模块级常量**。像游戏清单、导航项这些原来是 `const`,现在改成了函数(`GAMES()`、`nav()`)——
  常量会在模块加载那一刻就把当时的语言定死。

**后端返回的错误信息目前只有中文**(比如"未配置 OPENROUTER_API_KEY"),会原样显示在后台。
Go 那边没有做 i18n,要做的话是另一件事。
...
侧栏 🔨 那一支,三套地鼠:**1–9**、**Monday–Sunday**、**January–December**。
一轮:念一个英文词 → 在地里找到写着它的那只地鼠 → 抡锤子敲。

- **每只地鼠的洞位固定不变**,1 号永远在左上。孩子记的是"听到 seven 去中间那排最右",每题重排会把这条空间线索毁掉,所以位置只在窗口尺寸变化时重算
- 地鼠身上写的是本地化的短标签(中文界面 `周三` / `3月`,英文界面 `Wed` / `Mar`),念出来的始终是英文,练的是**听音 → 意思**
- 打错:红闪 + 那只缩回去 + 扣分 + **自动重播发音**,这一轮不结束;卡太久(默认 7 秒)正确的那只会亮一圈提示光
- 干扰地鼠自己起起落落,三档难度只改节奏和题量,洞的数量和位置不动
- `空格` 或点屏幕下方那条提示条重听,`Esc` 退出

发音默认走浏览器 TTS。想换真人发音,就在后台把 `one`…`nine`、`monday`…、`january`… 这些词条生成出来(图+音都齐,和普通词一样),打地鼠会自动认领同名词条的 mp3 —— 见 [voices.ts](web/src/whack/voices.ts)。

节奏参数都在 [timing.ts](web/src/whack/timing.ts):冒头/缩回速度、干扰鼠的起落区间、提示光出现的时间。
...
├── whack/                打地鼠:sets.ts(三套词) WhackScene(玩法) draw.ts(地鼠和锤子)
