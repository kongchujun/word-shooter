...
export interface NetPlayer { id:string; team:ArenaTeam; number:number; x:number;y:number;z:number;yaw:number;pitch:number;hp:number;dead:boolean;moving:boolean;weapon:'smg'|'sniper' }
...
export class ArenaOnline {
  ...
  constructor(readonly team:ArenaTeam, private state:()=>Omit<NetPlayer,'id'|'team'|'number'|'hp'|'dead'>) {}
