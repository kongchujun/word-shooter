...
export class ArenaOnline {
  ...
  onPlayers:(players:NetPlayer[])=>void=()=>{}; onRespawn:(p:NetPlayer)=>void=()=>{}; onError:()=>void=()=>{}
  ...
  private async sync():Promise<void>{ if(this.stopped||this.busy||!this.id)return;this.busy=true;try{const r=await fetch('/api/arena/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:this.id,token:this.token,...this.state()})});if(!r.ok)throw Error();const j=await r.json();const me=(j.players as NetPlayer[]).find(p=>p.id===this.id);if(me?.dead===false&&me.hp===100&&Math.abs(me.x-this.state().x)>30)this.onRespawn(me);this.onPlayers(j.players)}catch{this.onError()}finally{this.busy=false}}
}
